...
% ---------- Logo (SVG) ----------
logoPath = fullfile(assets,'logo.svg');

logoPath = fullfile(assets,'logo.svg');
if ~exist(logoPath,'file')
    writeLogo(logoPath, site.title);  % only create a placeholder if none exists
end
...
content.models = htmlList({ ...
  ...
  mklink('models_empirical.html','Empirical model'), ...
  mklink('models_adcirc.html','ADCIRC'), ...
  mklink('models_delft3d.html','Delft3D'), ...
  mklink('models_funwave.html','FUNWAVE-TVD'), ...
  mklink('models_nearcom.html','NearCoM'), ...
  mklink('models_xbeach.html','XBEACH'), ...
  mklink('models_cshore.html','CSHORE') ...
});
...
% ===== End of main; helpers below as nested functions =====
    ...
    function writePage(outDir_, assets_, site_, slug, title, body, pages)
        html = sprintf(['<!DOCTYPE html><html lang="en"><head>' ...
          ...
          '<img src="assets/logo.svg" alt="Logo"><span>%s</span></a></header>' ...
          ...
          '</body></html>'], ...
          title, site_.title, site_.title, topNav(pages, slug), sideNav(pages, slug), title, body, year(datetime('now')), site_.org);
