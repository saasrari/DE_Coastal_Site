...
% end
function writeLeafletMap(outDir_, assets_)
    % Writes docs/map_embed.html (Leaflet map with small numbered markers)
...
'  .num-marker{background:transparent;}'
'  .num-pin{width:20px;height:20px;border-radius:50%;background:#2563eb;border:2px solid #fff;'
'           box-shadow:0 0 0 1px rgba(0,0,0,.25);display:flex;align-items:center;justify-content:center;}'
'  .num-pin span{color:#fff;font:11px/1 system-ui,-apple-system,Segoe UI,Arial;}'
'</style></head><body>'
'<div id="map"></div>'
...
'  fetch("assets/geo/latest_run.geojson?v="+Date.now())'
'    .then(r => { if(!r.ok) throw new Error("latest_run.geojson not found"); return r.json(); })'
'    .then(g => {'
'      const layer = L.geoJSON(g, {'
'        pointToLayer: (feature, latlng) => {'
'          const name = String((feature.properties && feature.properties.name) || "");'
'          const m = name.match(/\\d+/);'  % extract "01" etc from "Profile 01"
'          const num = m ? m[0] : "";'
'          const icon = L.divIcon({'
'            className: "num-marker",'
'            html: "<div class=\\"num-pin\\"><span>"+num+"</span></div>",'
'            iconSize: [20,20],'
'            iconAnchor: [10,10],'
'            popupAnchor: [0,-10]'
'          });'
